import { AlertCircle, Info, Loader2, Plus, Save, Settings2, Trash2, X } from 'lucide-react'
...
export default function FeeSettings() {
  const { feeConfig, feeConfigLoading, feeConfigError, updateFeeConfig, addFeeCategory, deleteFeeCategory } =
    useApp()
  ...
  const key = classKey(selectedGrade, selectedProgram)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Keep the draft and `dirty` exactly as the staff left them — nothing was confirmed
      // saved, so the form must not silently reset or look like it succeeded.
  ...
  const handleAddCategory = async () => {
    ...
    setAdding(true)
    try {
      await addFeeCategory(selectedGrade, selectedProgram, newCategory)
      setAddOpen(false)
  ...
  const handleDeleteCategory = async () => {
    ...
    setDeleting(true)
    try {
      await deleteFeeCategory(selectedGrade, selectedProgram, confirmDeleteCat.id)
